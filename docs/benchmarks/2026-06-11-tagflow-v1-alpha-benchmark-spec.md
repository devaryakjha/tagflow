...
The current matrix defines evidence lanes, not public claims. Any timing,
coefficient-of-variation, missed-frame, or relative-speed value is report-only
until a stable reference environment and explicit regression policy are
committed. The current allowed gates are collection and stability gates.

| Suite | Renderers | Fixtures | Metrics | Automation tier | Expected command | Current gate posture |
| --- | --- | --- | --- | --- | --- | --- |
| Parser microbench | Tagflow only | `smoke_short_html`, `ai_answer_rich`, `table_dense`, `large_article`, `deep_nested_lists`, `table_stress` | median us/op, p95 us/op, coefficient of variation, nodes/sec | PR CI + local | `dart run melos run benchmark:micro` | Smoke gate for JSON emission and fixture coverage only. CV and median deltas are report-only until a repeated same-environment parser policy exists. |
| Converter microbench | Tagflow only | parsed versions of all HTML fixtures | median us/op for conversion/build, widget count, element count | PR CI + local | `dart run melos run benchmark:render` | Smoke gate for successful conversion/build samples only. No percentage regression threshold yet. |
| Full build benchmark | Tagflow only | `ai_answer_rich`, `table_dense`, `large_article` | first stable pump ms, total build/layout ms, rebuild ms after theme noop, rebuild ms after content change | PR CI + local | Future extension of `benchmark:render` | Not currently a separate gate. Define fixture coverage and summary schema before enforcing anything. |
| Native frame + scroll benchmark | Tagflow, `flutter_html`, `flutter_widget_from_html` | `ai_answer_rich`, `large_article`, `table_dense`, `table_stress` | `FrameTiming` build p50/p90/p99/worst, raster p50/p90/p99/worst, GC counts, missed-frame-budget counts, scroll completion time | Local profile run, optional nightly CI on a stable runner | `dart run melos run benchmark:profile:baselines`, then summarize and check | Local reference collection gate only: selected cells complete, repeat count is met, failed runs are empty, and viewport guard passes when configured. Frame timings are report-only. |
| Table stress frame benchmark | Tagflow, `flutter_html`, `flutter_widget_from_html` | `table_stress` | same as above plus first-content-visible notes when available | Local profile run | Included in the default profile baseline matrix | Release-significant stability lane: exception, OOM, overflow, missing artifact, or failed scroll must be explained before release. No first-content-visible threshold yet. |
| Markdown product-shape comparison | Tagflow, `flutter_markdown_plus`, `markdown_widget` | `ai_answer_rich.md` | first stable pump ms, build p90, raster p90, text selection behavior check | Local + optional report CI | Future markdown fixture/profile lane | Future report-only comparison. No relative-speed gate until markdown fixtures, adapters, and fairness policy land. |
| Large document stability | Tagflow, `flutter_html`, `flutter_widget_from_html` | `large_article` | peak elapsed time to first content, scroll completion, exceptions, memory notes, GC churn | Local profile + manual DevTools | Included in the default profile baseline matrix | Release-significant stability lane: render and scroll must complete without crash or missing artifact. Memory growth remains manual/report-only until allocation evidence is automated. |
...
| Native JSON profile smoke | `tagflow_native_json` | `native_ai_answer`, `native_table_dense`, `native_large_article` | profile frame summaries for `coldInitialRender`, `warmRebuild`, and `warmScroll`, plus viewport metadata from the example app after decoding native block JSON into `TagflowDocument` | Local report-only smoke | `TAGFLOW_PROFILE_PAIR=tagflow_native_json:native_ai_answer,tagflow_native_json:native_table_dense,tagflow_native_json:native_large_article TAGFLOW_PROFILE_REPEAT=1 dart run melos run benchmark:profile:baselines` | Report-only. Completion proves the native JSON render path is wired across trusted native document shapes; it is not a numeric threshold, public claim, or direct comparison with HTML parser/render lanes. |
