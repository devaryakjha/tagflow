...
| Authored insertion class-growth review | [`baselines/2026-06-12-authored-insertion-class-growth-review.md`](baselines/2026-06-12-authored-insertion-class-growth-review.md) | Existing VM-service exports were reviewed for class-level growth. Same-process patch aggregate object count and heap-summary shallow size did not grow from `before_first_patch` to `after_scroll`; package-level Tagflow growth was limited to one `TagflowDocumentNode` and one `TagflowDocument`. | Report-only class-growth interpretation only; public claims remain blocked without raw DevTools heap/diff review. |
...
- Memory/allocation: GC counts are captured in profile summaries, and a bounded
  ...
  claims until `tagflow:table_stress` raw heap/diff evidence is collected and
  the reviewed interpretation scope is decided for the required HTML memory
  lanes.
