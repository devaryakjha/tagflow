...
| Kite profile blocker | [`baselines/2026-06-12-kite-ipo-profile-evidence-blocked.md`](baselines/2026-06-12-kite-ipo-profile-evidence-blocked.md) | Kite now has hosted-alpha widget evidence, but no credible production profile capture is available because production IPO rendering still uses the legacy converter bridge, no deterministic production fixture opener exists, and physical target tooling is inconsistent. The next scoped production migration candidate is content-only IPO rendering through `Tagflow.html(..., registry: ...)` while keeping excerpt rendering on the legacy bridge. | Negative qualification evidence; real-app profile baseline still pending. |
...
- Real-app profile: hosted-alpha widget evidence exists in Kite, including
  ...
  opener exists, and physical-device tooling is inconsistent. A narrower
  content-only production migration slice is now queued, but it is not profile
  evidence until validated on a supported target.
