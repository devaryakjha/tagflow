...
| Memory snapshot blocker | [`baselines/2026-06-12-memory-allocation-snapshot-blocker.md`](baselines/2026-06-12-memory-allocation-snapshot-blocker.md) | The next non-device gap is not another bounded memory sample; it is DevTools checkpoint snapshots, class allocation diffs, or a harness change that preserves the VM service URI for reviewed capture. | Allocation evidence still blocked. |
